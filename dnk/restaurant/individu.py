...
class Individu:
    ...
    def test_despawn(self, grille):
        (x, y) = self.position
        return (
            grille[x][y][Constantes.nom_biblio_sprite]
    ...
    def deplacer(self, restaurant, liste_pos_clients):
        if len(self.chemin) != 0:
            ...
            if not (
                (position_suivante[0], position_suivante[1])
                in liste_pos_clients
            ):
    ...
    def set_position_but(
        self, restaurant, but, liste_position_personnages, est_personnel
    ):
        self.coordonnees_objectif = None
                ...
                if not objectif_trouve:
                    if but == Constantes.sprite_tapis_1:
                        ...
                        if (
                            ...
                            if not (
                                restaurant.grille_meubles[x][y][
                                    Constantes.nom_biblio_est_objectif
                                ]
                            ):
                        ...
                        if (
                            ...
                            == Constantes.sprite_machine_kebab_1
                        ):
                            self.coordonnees_objectif = (x, y + 1)
                            self.orientation_but = "UP"
                            objectif_trouve = True
    ...
    def test_set_objectif(
        self, pos, grille_meubles, liste_position_personnages
    ):
        test = False
        (x, y) = pos
        if not ((x, y) in liste_position_personnages):
    ...
    def modifier_chemin(
        self, restaurant, but, liste_position_personnages, est_personnel=False
    ):
        if self.coordonnees_objectif is None:
            self.set_position_but(
                restaurant, but, liste_position_personnages, est_personnel
            )
            if self.coordonnees_objectif is None:
                return False
        if self.coordonnees_objectif is not None:
            if self.position != self.coordonnees_objectif:
                if self.objectif == Constantes.sprite_caisse:
                    self.chemin = pathfinding(
                        restaurant.grille_meubles,
                        self.position,
                        self.coordonnees_objectif,
                        [],
                    )
                else:
                    self.chemin = pathfinding(

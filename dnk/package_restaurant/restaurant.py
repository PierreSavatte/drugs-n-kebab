...
class Restaurant:
    ...
    def __init__(self, grille):
        ...
        self.ressources = {
            Constantes.nom_pain: 10,
            Constantes.nom_viande_kebab: 10,
            Constantes.nom_oignon: 10,
            Constantes.nom_tomate: 10,
            Constantes.nom_salade: 10,
            Constantes.nom_galette: 10,
            Constantes.nom_patate: 0,
            Constantes.nom_algerienne: 10,
            Constantes.nom_samourai: 10,
            Constantes.nom_sauce_blanche: 10,
            Constantes.nom_mayonnaise: 10,
            Constantes.nom_ketchup: 10,
            Constantes.nom_pate_pizza: 1,
            Constantes.nom_fromage: 10,
            Constantes.nom_tomate: 10,
            Constantes.nom_creme: 10,
            Constantes.nom_thon: 10,
            Constantes.nom_saumon: 10,
        }
        return

    def cuisine(self):
        if self.commande_geree is not None:
    ...
    def test_ressources_suffisantes(self, commande_client, sauce_client):
        ...
        for nom_ressource in ressources_commande:
            if nom_ressource != Constantes.nom_sauce:
                test = (
                    test
                    and ressources_commande[nom_ressource]
                    <= self.ressources[nom_ressource]
                )
            else:
                test = (
                    test
                    and ressources_commande[nom_ressource]
                    <= self.ressources[sauce_client]
                )
        return test
    ...
    def get_positions_caisses(self):
        ...
        for x in range(0, len(self.grille_meubles)):
            for y in range(0, len(self.grille_meubles[0])):
                if (
                    self.grille_meubles[x][y][Constantes.nom_biblio_sprite]
                    == Constantes.sprite_caisse
                ):
    ...
    def gestion_client_en_caisse(
        ...
        temps,
        joueur=None,
        action_du_joueur=False,
    ):

        if biblio_caisse is not None:
            ...
            if (
                ...
                if action_du_joueur:
                    if joueur.action_termine:
                        commande_effective = affichage.mini_fenetre_commande(
                            self, temps, commandes_client, fenetre, font
                        )
                        if commande_effective is not None:
                            self.recevoir_commande(
                                commande_effective, commande_sauce
                            )
                    ...
                    while i < len(commandes_client):
                        if self.test_ressources_suffisantes(
                            commande_effective, commande_sauce
                        ):
                            commande_effective = commandes[i]

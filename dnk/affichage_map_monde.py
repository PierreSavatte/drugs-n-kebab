...
def redim_sprite_dim_fenetre(sprite, dimensions_fenetre):
    ...
    rapports = (
        dimensions_fenetre[0] * 1.0 / dimensions_sprite[0],
        dimensions_fenetre[1] * 1.0 / dimensions_sprite[1],
    )
    if rapports[0] < 1 or rapports[1] < 1:
        rapport = min(rapports[0], rapports[1])
        dimensions_sprite_prime = (
            int(rapport * dimensions_sprite[0]),
            int(rapport * dimensions_sprite[1]),
        )
...
def gest_mouse(fenetre, font, bibliotheque_villes):
    ...
    for nom_ville in bibliotheque_villes:
        ...
        rect_bouton_ville = pygame.Rect(
            pos_ville, Constantes.taille_sprite_point
        )
...
def gest_vehicule(fenetre, font, bibliotheque_villes, tabl_vehicule, temps):
    if random.randint(0, 100) >= 98 and not (temps.est_en_pause()):
        ...
        while ville_fin == ville_depart:
            ville_fin = main_fonctions.get_val_aleatoire_parmis(
                tabl_nom_villes
            )
        nom_ligne = ville_depart + " - " + ville_fin
        tabl_vehicule.append(
            Vehicule(
                nom_ligne,
                bibliotheque_villes[ville_depart],
                bibliotheque_villes[ville_fin],
                Constantes.sprite_avion,
                temps,
            )
        )
    indice_vehicule = 0
    for vehicule in tabl_vehicule:
        ...
        indice_vehicule += 1
        vehicule.gest_affichage_info(
            main_fonctions.get_pos_mouse(), fenetre, font
        )
    ...
    if temps.est_en_pause():
        if affichage_pause:
            ...
            fenetre.blit(
                surface_pause,
                (
                    taille_fenetre[0] / 2 - taille_surface_pause[0] / 2,
                    taille_fenetre[1] - 1.5 * taille_surface_pause[1],
                ),
            )
        if temps_affichee is None:
...
def affichage_map(fenetre, temps, mode_devellopeur=False):
    ...
    while not end:
        ...
        tabl_vehicule = gest_vehicule(
            fenetre, font_mouse, bibliotheque_villes, tabl_vehicule, temps
        )
        (affichage_pause, temps_affichee) = affichage_temps(
            fenetre, temps, affichage_pause, temps_affichee
        )
        pygame.display.flip()
    return
...
class Vehicule:
    def __init__(
        ...
        self.distance = main_fonctions.distance_entre_deux_points(
            pos_depart, pos_arrive
        )
        self.temps_deplacement = self.distance / Constantes.vitesse_vehicule
        self.transparent = transparent
        self.sprite = self.get_sprite(sprite, transparent)
        self.rect_arrivee = pygame.Rect(
            self.pos_arrive, Constantes.taille_sprite_point
        )
    ...
    def get_postion(self, temps):
        self.ratio = (temps.get_time() - self.temps_depart) / (
            self.temps_arrrive - self.temps_depart
        )
    ...
    def test_fin_deplacement(self):
        ...
        constante_fin = 2
        return (x_fin + constante_fin >= x >= x_fin - constante_fin) and (
            y_fin + constante_fin >= y >= y_fin - constante_fin
        )  # (self.rect.colliderect(self.rect_arrivee) == 1)
    ...
    def gest_affichage_info(self, pos_mouse, fenetre, font):
        if self.rect.collidepoint(pos_mouse):
            draw_mini_fenetre(
                ...
                font,
                self.position,
                self.nom_ligne + " : " + str(self.place_stockage),
            )
        return

    def calcul_angle(self):
        norme = main_fonctions.distance_entre_deux_points(
            self.pos_depart, self.pos_arrive
        )
        ...
        if Angle_sin / math.fabs(Angle_sin) == Angle_cos / math.fabs(
            Angle_cos
        ):
            return Angle_cos
        else:

...
@pytest.fixture
def restaurant(restaurant_size):
    return Restaurant(restaurant_size)


def test_restaurant_takes_a_model_to_be_init(restaurant):
...
def test_restaurant_loads_good_layers_depending_on_the_size(
    load_restaurant_file_mocked, _, restaurant
):
    size_name = restaurant.size.value
...
def test_restaurant_holds_player(restaurant):
    restaurant_scene = RestaurantScene(restaurant)
...
def test_restaurant_holds_all_the_layers(restaurant):
    restaurant_scene = RestaurantScene(restaurant)
...
def test_restaurant_define_good_layers_as_collidable(restaurant):
    restaurant_scene = RestaurantScene(restaurant)

...
class Personnel(Individu):
    ...
    #Constructeur:
    def __init__(self,couleur,sexe,categorie_arg,biblios_caisses):
        """
        ...
        self.categorie_a_afficher = self.def_categorie_a_afficher(categorie_arg,sexe)
        ...
        return Individu.__init__(self, couleur, sexe)
    ...
    def get_ordre(self,stat,restaurant):
        if not(self.action):
            ...
            self.stat_liee = stat
            retourne = self.modifier_chemin(restaurant,objectif_temp,Individu.tabl_positions,est_personnel = True)
            
            if len(self.chemin) != 0:
                pos_employe_en_caisse = self.chemin[-1]
                self.biblio_caisse_liee = self.get_biblio_caisse_liee(pos_employe_en_caisse)
            ...
            if self.position == self.coordonnees_objectif:
                if not(self.action_termine):
                    if self.stat_liee == Constantes.nom_stat_cuisine:
                        if restaurant.commande_geree != None:
                            self.debut_action(temps)
                        else:
                            print("En attente d'une commande")
                    if self.stat_liee == Constantes.nom_stat_vente :
                        restaurant.gestion_client_en_caisse(self.biblio_caisse_liee,fenetre,font,temps)
                        if self.biblio_caisse_liee[Constantes.nom_biblio_client_geree_en_caisse]:
                            self.debut_action()
                        else:
    ...
    def level_up(self,nom_stat):
        if nom_stat in Constantes.tabeau_noms_stats:
            self.statistiques[nom_stat] += int(5000/self.statistiques[Constantes.nom_stat_cuisine])
        return
    ...
    def def_categorie_a_afficher(self,categorie_arg,sexe):
        ...
        if sexe == Constantes.nom_homme:
            if categorie_arg == None:
                return self.main_fonctions.get_val_aleatoire_parmis(Constantes.noms_categorie_autres)
            else:
    ...
    def def_stats(self,categorie_arg):
        ...
        if categorie_arg == Constantes.nom_categorie_cuisinier:
            for i in range(len(Constantes.tabeau_noms_stats)) :
                caracteristiques[Constantes.tabeau_noms_stats[i]] = main_fonctions.get_valeur_multiple_de_5(random.randint(Constantes.tabeau_valeurs_stats_cuisinier[i][0],Constantes.tabeau_valeurs_stats_cuisinier[i][1]))
        elif categorie_arg == Constantes.nom_categorie_chef_cuisinier:
            for i in range(len(Constantes.tabeau_noms_stats)):
                caracteristiques[Constantes.tabeau_noms_stats[i]] = main_fonctions.get_valeur_multiple_de_5(random.randint(Constantes.tabeau_valeurs_stats_chef_cuisinier[i][0],Constantes.tabeau_valeurs_stats_chef_cuisinier[i][1]))
        elif categorie_arg == Constantes.nom_categorie_caissier:
            for i in range(len(Constantes.tabeau_noms_stats)):
                caracteristiques[Constantes.tabeau_noms_stats[i]] = main_fonctions.get_valeur_multiple_de_5(random.randint(Constantes.tabeau_valeurs_stats_caissier[i][0],Constantes.tabeau_valeurs_stats_caissier[i][1]))
        else:
            for i in range(len(Constantes.tabeau_noms_stats)):
                caracteristiques[Constantes.tabeau_noms_stats[i]] = main_fonctions.get_valeur_multiple_de_5(random.randint(Constantes.tabeau_valeurs_stats_None[i][0],Constantes.tabeau_valeurs_stats_None[i][1]))

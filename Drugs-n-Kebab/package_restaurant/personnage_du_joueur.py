...
class Perso_du_joueur:
    ...
    def __init__(self,nom_joueur):
        ...
        self.liste_recettes_debloquees = {Constantes.nom_kebab_simple:[Constantes.nom_pain]}
        self.satisfaction = 0
        self.notoriete = 0
    ...
    def deplacer_joueur (self,touche_deplacement,grille):
        ...
        if position_deplacee != self.position:
            if position_deplacee[0] >=0 and position_deplacee[0] < len(grille):
                if position_deplacee[1] >=1 and position_deplacee[1] < len(grille[0]):
                    if grille[position_deplacee[0]][position_deplacee[1]][Constantes.nom_biblio_walkable]:
    ...
    def get_biblio_caisse(self,biblios_caisses,position_caisse):
        for biblio_caisse in biblios_caisses:
            if biblio_caisse[Constantes.nom_biblio_position] == position_caisse:
    ...
    def update_biblio_caisse(self,temps,biblio_caisse):
        ...
        biblio_caisse[Constantes.nom_biblio_temps_depart_client_geree_en_caisse] = temps.get_time()
        return
    ...
    def gestion_actions(self,temps,touche_clavier,restaurant):
        if not(self.action):
            if touche_clavier == pygame.K_e:
                ...
                if restaurant.grille_meubles[x][y][Constantes.nom_biblio_sprite] == Constantes.sprite_caisse and self.test_presence_client(restaurant,(x,y+1)):
                    self.action_liee = "vente"
                    self.action = True
                    self.debut_temps_action = temps.get_time()
                    self.update_biblio_caisse(temps,self.get_biblio_caisse(restaurant.pos_caisses,(x,y)))
                elif restaurant.grille_meubles[x][y][Constantes.nom_biblio_sprite] in Constantes.liste_cuisines and restaurant.commande_geree != None :
                    self.action_liee = "cuisine"
                    self.action = True
                    self.debut_temps_action = temps.get_time()
        else:
            if self.action_liee == "vente":
                if temps.get_time() >= self.debut_temps_action + Constantes.delai_gestion_client_en_caisse:
                    self.action = False
                    self.debut_temps_action = None

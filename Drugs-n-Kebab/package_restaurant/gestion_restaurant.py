...
def loop(fenetre,font,restaurant,joueur,temps,pas_de_clients = False):
    ...
    """
    restaurant.employes.append(Personnel(Constantes.nom_arabes,Constantes.nom_homme,Constantes.nom_categorie_chef_cuisinier,restaurant.pos_caisses))
    ...
    while not(quitter):
        ...
        if temps_deplacement + Constantes.delai_deplacement_client <= temps.get_time() and not(temps.est_en_pause()):
            temps_deplacement = temps.get_time()
            gest_depl_individus(restaurant)
            
            for biblio_caisse in restaurant.pos_caisses:
                restaurant.gestion_client_en_caisse(biblio_caisse,fenetre,font,temps,joueur,action_du_joueur = True)
            
            for client in restaurant.clients:
                client.Update(temps,restaurant)
        
        (boutons,boutons_affichees) = Gestion_boutons_ordres_employes(restaurant,fenetre,temps,font,pos_mouse_clic,boutons_affichees,boutons)
        ...
        affichage.affichage_restaurant(restaurant,fenetre,font,joueur,boutons,temps)
        quitter = test_fin_loop(coord_sortie,joueur)
    return

def Gestion_boutons_ordres_employes(restaurant,fenetre,temps,font,pos_mouse_clic,boutons_affichees,boutons):
    if pos_mouse_clic != None:
            if not(boutons_affichees):
                coord_mouse = affichage.position_en_coordonnees(pos_mouse_clic,affichage.get_taille_sprite(restaurant.grille_meubles))
                for employe in restaurant.employes:
                    if employe.position == coord_mouse:
                        boutons = Boutons_actions_employes(font,employe,pos_mouse_clic,restaurant,temps)
...
def compter_chaises(restaurant):
    ...
    for x in range(0,len(restaurant)):
        for y in range(0,len(restaurant[0])):
            if restaurant[x][y][Constantes.nom_biblio_sprite] in Constantes.sprites_chaise:
                compteur += 1
    return compteur
...
def get_coord_sortie(grille_meubles):
    for x in range(0,len(grille_meubles)):
        for y in range(0,len(grille_meubles[0])):
            if grille_meubles[x][y][Constantes.nom_biblio_sprite] == Constantes.sprite_tapis_1:
                return (x,y)
    return
    ...
    for entitees in [restaurant.clients,restaurant.employes]:
        ...
        if len(entitees)!=0:
            for entitee in entitees:
                ...
                entitee.modifier_chemin(restaurant,entitee.objectif,get_liste_pos_pathfinding(restaurant.clients))
                entitee.deplacer(restaurant,get_liste_pos_pathfinding(restaurant.clients))
...
def spawn_clients(liste_clients):
    ...
    liste_clients.append(Client(get_valeur_alea(Constantes.tableau_couleurs),get_valeur_alea(Constantes.tableau_sexe),liste_recettes_debloquees))
...
def calcul_nombre_maximum_clients(heure,restaurant,joueur):
    ...
    nb_maximum_clients = 1 + temps_int+(satisfaction_int + proprete_int) + int(notoriete/1000)
...
def calcul_parametre_temps(temps):
    ...
    heures = int(temps[:2])
    temps_int = 1
    if ((heures>=11 and heures <=13) or (heures>=19 and heures <=21)):
        temps_int = 2
    return temps_int

def parametre_en_entier(parametre):
    "Cette fonction est utilisee pour passer les parametres de proprete et de satisfaction, qui sont comptes entre 0 et 100, en entiers comptes entre 0 et 6"
    parametre_int = 0

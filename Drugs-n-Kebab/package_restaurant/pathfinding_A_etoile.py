...
def get_noeud_plus_bas_cout_grille_ouverte(coord_depart,grille_ouverte,coord_fin):
...
def get_path(grille_fermee,coord_fin,coord_depart):
    ...
    while coord_etudiee != coord_depart:
        ...
        coord_etudiee = grille_fermee[get_indice_item(grille_fermee,coord_etudiee)][1]
...
def pathfinding(grille_restaurant,coord_depart,coord_fin,liste_position_personnages):
    
    if coord_depart == None or coord_fin == None:
        print("ERROR : "+ str(coord_depart)+" ; "+ str(coord_fin))
    
    grille_ouverte = []
    grille_fermee = []
    
    grille_ouverte.append((coord_depart,None,get_cout(coord_depart,coord_depart,coord_fin)))
    
    coord_actuelle = coord_depart
    
    profondeur = 0
    
    while (coord_actuelle != coord_fin) and (profondeur <= Constantes.profondeur_pathfinding):
        noeud_plus_bas = get_noeud_plus_bas_cout_grille_ouverte(coord_depart,grille_ouverte,coord_fin)
        coord_actuelle = noeud_plus_bas[0]
        pop_valeur_in_liste(coord_actuelle,grille_ouverte)
        grille_fermee.append(noeud_plus_bas)
        
        for (x,y) in [(coord_actuelle[0]-1,coord_actuelle[1]),(coord_actuelle[0],coord_actuelle[1]-1),(coord_actuelle[0],coord_actuelle[1]+1),(coord_actuelle[0]+1,coord_actuelle[1])]:
            if x >= 0 and x < len(grille_restaurant) and y >= 0 and y < len(grille_restaurant[0]) and (x,y) != coord_actuelle :
                voisin = (x,y)
                if grille_restaurant[x][y][Constantes.nom_biblio_walkable] and  not(test_in_list(grille_fermee,voisin)) and not ((x,y)in liste_position_personnages):
                    cout_temp = get_cout(coord_depart, voisin, coord_fin)
                    i = get_indice_item(grille_ouverte,voisin)
                   
                    if i == None:  #if not(test_in_list(grille_ouverte,voisin)):
                        grille_ouverte.append((voisin,coord_actuelle,cout_temp))

...
class Client(Individu):
    ...
    def __init__(self,couleur,sexe,liste_recettes_debloquees):
        self.money = 0
    ...
    def Update(self,temps,restaurant):
        if self.test_si_mange():
            if self.coordonnees_objectif==self.position:
                if self.temps_depart_repas == None:
    ...
    def test_si_fini_manger(self,temps):
        return (temps.get_time() >= self.temps_depart_repas +Constantes.delai_repas_client)
    ...
    def choix_commande(self,liste_recettes_debloquees):
        self.commande_sauce = main_fonctions.get_val_aleatoire_parmis(Constantes.sauces)
        self.commande_1 = main_fonctions.get_val_aleatoire_parmis_bibliotheque(liste_recettes_debloquees)
        commande_2 = main_fonctions.get_val_aleatoire_parmis_bibliotheque(liste_recettes_debloquees)

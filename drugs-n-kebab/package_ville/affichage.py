...
#from main_loop import get_val_aleatoire_parmis
from package_ville.groupe_sprite import Groupe_sprite

def coordonnees_en_position(coordonnees):
    (x,y) = coordonnees
    return ((x-y)*(1/2*Constantes.taille_sprite_ville_x)+1/2*Constantes.taille_fen[0],(y+x)*(1/2*Constantes.taille_sprite_ville_y+.5)+Constantes.taille_sprite_ville_y)

def position_en_coordonnees(position):
    (x,y) = position
    return (int(((1/2*Constantes.taille_fen[0])/Constantes.taille_sprite_ville_x)-(x/Constantes.taille_sprite_ville_x)+(3/(Constantes.taille_sprite_ville_y+1))*y)-2,int(-(x*(1/Constantes.taille_sprite_ville_x)-(1/((Constantes.taille_sprite_ville_y+1)))*y-((1/2*Constantes.taille_fen[0])/Constantes.taille_sprite_ville_x)-Constantes.taille_sprite_ville_y/(1/2*Constantes.taille_sprite_ville_y+.5)))-2)

def add_sprites_sol(groupe):
    for x in range(Constantes.taille_ville[0]+1):
        for y in range(Constantes.taille_ville[1]+1):
            add_sprite(groupe,[Constantes.nom_sprite_terrain],(x,y), sol = True)
    return
...
def add_sprite(groupe,objet_batiment,coordonnees,sol = False):
    ...
    sprite.rect.midleft =(position[0],position[1]-1/2*(taille_image[1]-Constantes.taille_sprite_ville_y))
    if sol:
        groupe.add(sprite)
    else:
...
def applique_deplacement(rect,tabl_deplacement):
    return pygame.Rect(rect.left+tabl_deplacement[0],rect.top+tabl_deplacement[1],rect.width,rect.height)

def gest_deplacement_map(fenetre,groupe_sol,groupe_batiment,temps_dernier_mvt,acceleration,old_mouvement):
    ...
    (tabl_deplacement,temps_dernier_mvt) = main_fonctions.get_rect_mouvement(orientation_mouvement,temps_dernier_mvt,acceleration)
    ...
    for i in range(len(groupe_sprites)):
        ...
        if sprite.rect.collidepoint(pos_mouse):
            ...
            if not(est_bat_deco):
                ...
                affichage_mini_fenetre_info(fenetre,font,(pos_mouse[0]+10,pos_mouse[1]+10),stat_bat)
                break
    ...
    if batiment != None :
        if not(batiment[0]in Constantes.noms_sprites_deco):
            ...
            for stat in batiment[1]:
                ...
                texte = font.render(str(stat) +" = "+str(batiment[1][stat]),False,(0,0,0))
                textes.append(texte)
                long_texte = texte.get_width()
                if long_texte > long_max:
                    long_max = long_texte
            i = 0
            rect = (pos,(long_max+10,(len(textes)+.5)*font_height+10))
...
def affichage_ville(font,ville):
    ...
    while not(end) :
        ...
        (temps_dernier_mvt,old_mouvement,acceleration) = gest_deplacement_map(fenetre,groupe_sol,groupe_batiment,temps_dernier_mvt,acceleration,old_mouvement)

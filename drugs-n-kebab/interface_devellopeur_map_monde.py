...
import pygame
from .main_fonctions import affichage_texte,get_pos_clic,Slider
"""
...
def affichage_sliders_test(fenetre,font):
    ...
    for i in range(10):
        if i ==1:
            val_depart = 100
            sliders.append(Slider((40,170),val_depart = val_depart,val_max = 100, nom_slider='',horizontal_arg = True))
        else:
            val_depart = 0
            sliders.append(Slider(pos_slider,val_depart = val_depart,val_max = 100, nom_slider='',horizontal_arg = False))
...
def gestion_outils(fenetre,font,curseurs,mouse):
    ...
    for i in range(len(curseurs)):
        affichage_texte(fenetre,font,Constantes.tableau_couleurs[i][:6],(pos_col(i)[0],5))
        rect = pygame.Rect(pos_col(i),(Constantes.taille_slide_x,Constantes.taille_slide_y))
        pygame.draw.rect(fenetre, (0,0,0) ,rect ,1)
        pygame.draw.rect(fenetre,(0,0,0),(percent_to_pos(curseurs[i],i),(Constantes.taille_curseur_x,Constantes.taille_curseur_y)))
...
def parcourir_bibliotheque_et_ajouter_stats(fenetre,font):
    ...
    for nom_ville in biblio:
        ...
        while not(end):
            ...
            affichage_texte(fenetre,font,str(int(nombre_faits/len(biblio) *100))+' %',(15,25))
            mouse = get_pos_clic()

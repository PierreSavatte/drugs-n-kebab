...
def get_rect_mouvement(orientation_mouvement,temps_dernier_mvt,acceleration):
    ...
    @return: (Rect,temps_dernier_mvt) :
        ...
        temps_dernier_mvt : float : temps du dernier mouvement a l'ecran, mis a jour
    """
    return move_rect(pygame.Rect(0,0,0,0),orientation_mouvement,temps_dernier_mvt,acceleration)
...
class Menu_Deroulant:
    ...
    """
    def __init__(self,font,position_rect,nom,tabl_fonctions_dans_deroulements):
        """
        Constructeur
        ...
        self.image_fleche = pygame.image.load(Constantes.sprite_fleche_menu_deroulant)
        
        self.main_bouton = Bouton(font,self.position_rect,self.nom,self.Derouler)
        bottomleft = self.main_bouton.rect.bottomleft
        self.position_rect_deroulement = (bottomleft[0]+self.image_fleche.get_rect().size[0]*2,bottomleft[1]+Constantes.decalage_bouton_main_deroulement_y)
        
        (self.tabl_boutons,self.rect_sous_boutons) = self.def_boutons(font,tabl_fonctions_dans_deroulements)
        return
    ...
    def def_boutons(self,font,tabl_fonctions_dans_deroulements):
        ...
        for i in range(len(tabl_fonctions_dans_deroulements)):
            ...
            tabl_boutons.append(Bouton(font,pos_temp,fonction.__name__ ,fonction))
            hauteur_bouton = tabl_boutons[i].rect.size[1]
            largeur_bouton = tabl_boutons[i].rect.size[0]
            pos_temp_1 = pos_temp[1]
            hauteur_total = Constantes.decalage_entre_boutons_y + hauteur_bouton
        ...
        size_rect[0] += self.position_rect_deroulement[0] -self.position_rect[0]
        rect_sous_boutons = pygame.Rect((self.position_rect[0],self.position_rect_deroulement[1]),size_rect)
    ...
    def Update(self,surface,pos_mouse_clic,pos_mouse):
        ...
        if self.deroulee :
            if pos_mouse != None:
                if not(self.rect_sous_boutons.collidepoint(pos_mouse)) and temps-self.temps_deroule >= Constantes.delai_clique_logo:
                    self.deroulee = False
            #pygame.draw.rect(surface,(0,0,0),self.rect_sous_boutons)
            for bouton in self.tabl_boutons:
                pos_bouton = bouton.position
                surface.blit(self.image_fleche,(pos_bouton[0]-self.image_fleche.get_rect().size[0],pos_bouton[1]+2))
                bouton.afficher(surface)
            if pos_mouse_clic != None:
                for bouton in self.tabl_boutons:
                    if (bouton.rect.collidepoint(pos_mouse) == 1):
                        if self.ancien_bouton != bouton or self.temps_dernier_clic + Constantes.delai_clique_logo <= temps:
...
class Slider:
    ...
    """
    def __init__(self,pos,val_depart = 0,val_max = 100,num_couleur_arg = None, nom_slider='',horizontal_arg = False):
        """
            Constructeur
    ...
    def apply_modif_valeurs(self,tabl_sliders_lies,valeur_temp):
        ...
        if tabl_sliders_lies == None:
            ...
            if nouvelle_val != ancienne_val:
                ...
                modif_pour_autres_curseurs = -difference/(len(curseurs_modifiables))
        ...
        if (pos_mouse != None):
            if self.rect_slider.collidepoint(pos_mouse):
                if self.horizontal:
                    ...
                    valeur_temp = int(((-(x-x_start)/self.taille_slide_y)+1)*self.valeur_max)
                    self.apply_modif_valeurs(tabl_sliders_lies,valeur_temp)
                else:
                    y = pos_mouse[1]
                    y_start = self.position_start_slider[1]
                    valeur_temp = int(((-(y-y_start)/self.taille_slide_y)+1)*self.valeur_max)
                    self.apply_modif_valeurs(tabl_sliders_lies,valeur_temp)
        return 
    ...
    def get_rect_slider(self):
        ...
        """
        if self.horizontal:
            rect = pygame.Rect(self.position_start_slider,(self.taille_slide_y,self.taille_slide_x))
        else:
            rect = pygame.Rect(self.position_start_slider,(self.taille_slide_x,self.taille_slide_y))
        return rect
    ...
    def get_rect_curseur(self):
        ...
        """
        if self.horizontal:
            x = (-(self.valeur/self.valeur_max)+1)*self.taille_slide_y + self.position_start_slider[0]-(.25*self.taille_curseur_x)
            y= self.position_start_slider[1]-(self.taille_curseur_y-self.taille_slide_x)/2 -(self.taille_curseur_y)
            rect =pygame.Rect((x,y),(self.taille_curseur_y,self.taille_curseur_x))
        else:
            x = self.position_start_slider[0]-(self.taille_curseur_x-self.taille_slide_x)/2
            y= (-(self.valeur/self.valeur_max)+1)*self.taille_slide_y + self.position_start_slider[1] -(.5*self.taille_curseur_y)
            rect =pygame.Rect((x,y),(self.taille_curseur_x,self.taille_curseur_y))
        return rect
    ...
    def afficher(self,surface,font):
        ...
        if self.nom != '':
            ...
            if self.horizontal:
                ...
                position = (self.position_start_slider[0]-size_surf[0],self.position_start_slider[1])
            else:
                size_surf = surf_nom.get_rect().size
                position = (self.position_start_slider[0]-size_surf[0]/4,self.position_start_slider[1]-size_surf[1])
            surface.blit(surf_nom,position)
        pygame.draw.rect(surface, (0,0,0), self.rect_slider, 1)
        if self.num_couleur != None:
            couleur_curseur = [0,0,0]
            if self.valeur <=255 and self.valeur>=0:
...
class Bouton:
    ...
    """
    def __init__(self,font,pos,nom,fonction_retour,resize = None,couleur_bouton = (255,255,255),actif = True):
        """
            Constructeur
    ...
    def get_sprite(self,font,resize):
        ...
        pygame.draw.rect(surface_sprite, Constantes.couleur_gris, ((0,0),(new_size[0]-2,new_size[1]-2)),2)
        pygame.draw.rect(surface_sprite, Constantes.couleur_gris_light, ((2,2),(new_size[0]-2,new_size[1]-2)),2)
        pygame.draw.rect(surface_sprite, (0,0,0), ((0,0),(new_size[0],new_size[1])),2)
        surface_sprite.blit(surface_texte,(5,5))
        if resize != None:
            taille_surface_sprite = surface_sprite.get_rect().size
            surface_sprite = pygame.transform.scale(surface_sprite, (int(taille_surface_sprite[0]*resize), int(taille_surface_sprite[1]*resize)))
    ...
    def Update(self,surface,pos_mouse_clic):
        ...
        if pos_mouse_clic != None and self.actif:
...
class Rect_Saisie:
    ...
    def __init__(self,pos,val_arg,val_max_arg):
        ...
        self.taille = (len(str(self.val_max))*Constantes.taille_police_ecriture,1.5*Constantes.taille_police_ecriture)
        self.rectangle_noir = pygame.Rect(pos,self.taille)
        self.rectangle_gris = pygame.Rect((pos[0]+1,pos[1]+1),(self.taille[0]-1,self.taille[1]-1))
        self.position_texte = (pos[0]+3,pos[1]+3)
        return
    
    def afficher(self,surface,font,couleur_valeur_saisie = (0,0,0),valeur_temp = None):
        """
        ...
        pygame.draw.rect(surface, Constantes.couleur_gris ,self.rectangle_gris,1)
    ...
    def mode_saisie(self,surface,font):
        ...
        if (mouse != None):
            ...
            saisie = False
        while saisie and not((key ==pygame.K_RETURN) or (key ==pygame.K_KP_ENTER)):
            pygame.draw.rect(surface, (255,255,255), self.rectangle_noir)
            self.afficher(surface,font,couleur_valeur_saisie = (255,0,0),valeur_temp = chaine_saisie)
            key = get_clavier()
            ...
            mouse = get_pos_clic()
            if mouse != None and temps_debut_mode_saisie+Constantes.delai_clique_logo <= time.time() :
                saisie = False
            pygame.display.flip()

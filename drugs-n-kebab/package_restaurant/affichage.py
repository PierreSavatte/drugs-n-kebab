...
def coordonnees_en_position(coordonnees,taille_sprite):
    ...
    """
    return (coordonnees[0]*taille_sprite[0] + Constantes.larg_cadre_interface,coordonnees[1]*taille_sprite[1])
...
def position_en_coordonnees(position,taille_sprite):
    ...
    @return: position dans la fenetre, tupple d'entiers
    """
    return (int((position[0]- Constantes.larg_cadre_interface)/taille_sprite[0] ),int(position[1]/taille_sprite[1]))
...
def affichage_rect(Surface,coordonnees,couleur):
    ...
    @param coordonnees: coordonnees du sprite a afficher dans le quadrillage
    """
    pygame.draw.rect(Surface, couleur, (coordonnees , Constantes.taille_bouton), 2)
    return
...
def affichage_bouton(Surface,sprite,position):
    ...
    """
    pygame.draw.rect(Surface, Constantes.couleur_rect_boutons, (position,Constantes.taille_bouton), width=1)
...
def add_sprites(groupe,nom_sprite):
    ...
    sprite_block.image.blit(image_sprite,((0,0),(16,16)))#fill((255,255,255))
...
def get_taille_sprite(grille_meubles):
    ...
    taille_sprite = min(int(taille_fen_x/taille_resto_x),int(taille_fen_y/taille_resto_y))
    return (taille_sprite,taille_sprite)

def affichage_restaurant(restaurant,fenetre,font,joueur,boutons_actions_employes,temps):
    ...
    for x in range(Constantes.taille_restaurant[0]):
        for y in range(Constantes.taille_restaurant[1]):
            if restaurant.grille_meubles[x][y][Constantes.nom_biblio_sprite]!= None:
                sprite = pygame.image.load(restaurant.grille_meubles[x][y][Constantes.nom_biblio_sprite])
                affichage_sprites(fenetre, redim_sprite(sprite,taille_sprite), coordonnees_en_position((x,y),taille_sprite))
                """
                ...
                """
    affichage_joueur(fenetre,temps,joueur,restaurant.grille_meubles,taille_sprite)
    if len(restaurant.clients)!=0:
        for client in restaurant.clients:
            affichage_client(fenetre,temps,client,restaurant.grille_meubles,taille_sprite,font)
    if len(restaurant.employes)!=0:
        for employe in restaurant.employes:
            affichage_employe(fenetre,employe,restaurant.grille_meubles,taille_sprite,font)
    for biblio in restaurant.pos_caisses:
        if biblio[Constantes.nom_biblio_client_geree_en_caisse]:
            ...
            temps_final = biblio[Constantes.nom_biblio_temps_depart_client_geree_en_caisse]+Constantes.delai_gestion_client_en_caisse
            affichage_barre_attente(fenetre,(x,y-1),biblio[Constantes.nom_biblio_temps_depart_client_geree_en_caisse],temps_final,time.time(),taille_sprite)
...
def affichage_fenetre_actions_avec_personnel(fenetre,boutons_actions_employes):
    if boutons_actions_employes != None:
        boutons_actions_employes.Update(fenetre,None)

def affichage_barre_attente(fenetre,position,temps_debut,temps_fin,temps_actuel,taille_sprite):
    long = ((temps_actuel-temps_debut)/(temps_fin-temps_debut))*taille_sprite[0]
    if long <= taille_sprite[0]:
        pygame.draw.rect(fenetre, (255,0,0), (coordonnees_en_position(position,taille_sprite), (long,Constantes.hauteur_barre_chargement)))
        pygame.draw.rect(fenetre, (0,0,0), (coordonnees_en_position(position,taille_sprite), (long,Constantes.hauteur_barre_chargement)), 2)
    return
...
def affichage_fond(fenetre,taille_sprite):
    ...
    for x in range(Constantes.taille_restaurant[0]):
        for y in range(Constantes.taille_restaurant[1]):
            affichage_sprites(fenetre, redim_sprite(fond_sol,taille_sprite), coordonnees_en_position((x,y),taille_sprite))
            if y < 2:
                affichage_sprites(fenetre, redim_sprite(fond_mur,taille_sprite), (x*taille_sprite[0] + Constantes.larg_cadre_interface,y*taille_sprite[1]-5))
    return
...
def affichage_joueur(fenetre,temps,joueur,grille,taille_sprite):
    ...
    affichage_sprites(fenetre, redim_sprite(pygame.image.load(joueur.get_sprite()),taille_sprite),(coordonnees[0],coordonnees[1]-5) )
    if joueur.action and joueur.action_liee != "vente":
        affichage_barre_attente(fenetre,joueur.position,joueur.debut_temps_action,joueur.debut_temps_action+Constantes.delai_gestion_client_en_caisse,temps.get_time(),taille_sprite)
    return
...
def affichage_client(fenetre,temps,client,grille,taille_sprite,font):
    ...
    sprite = redim_sprite(pygame.image.load(client.get_sprite()),taille_sprite)
    ...
    afficher_nom_individu(fenetre,font,client,coord_haut_droite_sprite_client,sprite)
    if client.test_si_mange():
        if client.coordonnees_objectif == client.position:
            affichage_barre_attente(fenetre,client.position,client.temps_depart_repas,client.temps_depart_repas+Constantes.delai_repas_client,temps.get_time(),taille_sprite)
    return

def afficher_nom_individu(fenetre,font,employe,coord_haut_droite_sprite_client,sprite):
    rect_sprite_client = pygame.Rect(coord_haut_droite_sprite_client,sprite.get_rect().size)
...
def affichage_employe(fenetre,employe,grille,taille_sprite,font):
    ...
    sprite = redim_sprite(pygame.image.load(employe.get_sprite()),taille_sprite)
    ...
    afficher_nom_individu(fenetre,font,employe,coord_haut_droite_sprite_employe,sprite)
    if employe.action:
        affichage_barre_attente(fenetre,employe.position,employe.temps_debut_action,employe.temps_debut_action+employe.get_temps_action(),time.time(),taille_sprite)
    return
...
def affichage_infos_recettes(fenetre,font,commande,position_rect,resize):
    ...
    surface_texte = font.render('Prix de vente : '+str(commande[1][1])+'$', True, (0,0,0))
    taille_texte = surface_texte.get_size()
    surface_texte = pygame.transform.scale(surface_texte, (int(taille_texte[0]*resize), int(taille_texte[1]*resize)))
    fenetre.blit(surface_texte,position_rect)
    position_rect = (position_rect[0],position_rect[1] + surface_texte.get_size()[1] + 5)
...
def mini_fenetre_commande(restaurant,temps,commandes,fenetre,font):
    ...
    position_rect = (int(taille_fenetre[0]/2 - 1.5*Constantes.larg_cadre_interface),int(taille_fenetre[1]/2 - 1.5*Constantes.larg_cadre_interface))
    larg_rect = (4*Constantes.larg_cadre_interface,2*Constantes.larg_cadre_interface)
    
    surface_texte = font.render('Gestion de la commande du client :', True, (0,0,0))
    ...
    if restaurant.test_ressources_suffisantes(commandes[0][0],commandes[1]):
        ...
        actif = False
    boutons.append(main_fonctions.Bouton(font,position_rect,commandes[0][0][0],return_commande_1,couleur_bouton = couleur_bouton_arg,actif = actif))
    position_rect = (position_rect[0],position_rect[1] + boutons[0].get_hauteur() + 5)
    
    position_rect = affichage_infos_recettes(fenetre,font,commandes[0][0],position_rect,resize)
    ...
    if restaurant.test_ressources_suffisantes(commandes[0][1],commandes[1]):
        ...
        couleur_bouton_arg = Constantes.couleur_rouge
        actif = False
    boutons.append(main_fonctions.Bouton(font,position_rect,commandes[0][1][0],return_commande_2,couleur_bouton = couleur_bouton_arg,actif = actif))
    position_rect = (position_rect[0],position_rect[1] + boutons[1].get_hauteur() + 5)
    
    position_rect = affichage_infos_recettes(fenetre,font,commandes[0][1],position_rect,resize)
    
    boutons.append(main_fonctions.Bouton(font,position_rect,"Faire sortir le client",return_quitter_resto,couleur_bouton = (255,255,255)))
    position_rect = (position_rect[0],position_rect[1] + boutons[2].get_hauteur() + 5)
    
    commande_selectionee = None
    temps.set_pause()
    while commande_selectionee == None:
